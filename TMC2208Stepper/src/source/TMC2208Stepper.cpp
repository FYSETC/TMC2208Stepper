...
TMC2208Stepper::TMC2208Stepper(SoftwareSerial * SerialPort, bool has_rx) {
	write_only = !has_rx;
	uses_sw_serial = true;
	SWSerial = SerialPort;
}
...
//fzl:add 20180801
bool TMC2208Stepper::microstepsRead() {
	CHOPCONF(&CHOPCONF_sr);
}
...
// 如果是读取的话，发送读命令之前要先listen这一路UART的rx，改变active_object
