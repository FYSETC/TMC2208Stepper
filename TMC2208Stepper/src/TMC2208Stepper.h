...
class TMC2208Stepper {
	public:
		TMC2208Stepper(SoftwareSerial * SerialPort, bool has_rx=true);
		//TMC2208Stepper(HardwareSerial * SerialPort, bool has_rx=true);
		...
		uint32_t getCHOPCONF_sr();//fzl
		void toff(uint8_t B);
		void toffRead(uint8_t B);//fzl:add 20180801
		...
		inline uint16_t get_bytes_written() __attribute__((always_inline)) { return bytesWritten;} //fzl:add 20180801
		inline bool is_software_serial() __attribute__((always_inline)) { return uses_sw_serial;} //fzl:add 20180802
		float Rsense = 0.11;
	...
	private:
		...
		void sendDatagram(uint8_t addr, uint32_t regVal, uint8_t len=7); // 写寄存器需要写入8个字节
		bool sendDatagram(uint8_t addr, uint32_t *data, uint8_t len=3); // 读寄存器只需要写入4个字节
		uint8_t calcCRC(uint8_t datagram[], uint8_t len);
		// Shadow registers fzl:下面的定义为 某个寄存器复位后的默认值
